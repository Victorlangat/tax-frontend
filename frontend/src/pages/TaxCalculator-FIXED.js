...
import * as taxCalc from '../services/taxCalculation.js';
...
const TaxCalculator = () => {
    ...
    if (savedVehicle) {
      try {
        ...
        if (!vehicleData.crsp) {
          ...
          const age = new Date().getFullYear() - vehicleData.year || 0;
          setTaxParams({
            isDirectImport: true,
  ...
  const calculateTax = () => {
    ...
    const calcResult = taxCalc.calculateVehicleTax(calculationInputs);

    const depreciationRate = taxCalc.getDepreciationRate(
      taxParams.isDirectImport ? 'direct' : 'prev', 
      parseInt(taxParams.age)
    ) || 0;
    const retentionPct = ((100 - depreciationRate) / 100 * 100).toFixed(0);
    ...
    const calculationResult = {
      ...
      breakdown: [
        { 
          label: `Customs Value (${calcResult.category.name}, Retention ${retentionPct}%)`, 
